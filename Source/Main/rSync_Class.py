...
class rSync_Class():
    ...
    def processFolder(self, node_name: str, folder: str, profile: dict, delete_excluded: bool=False, prompt: bool=True, dry_run='--dry-run'):
        ...
        if remote_path:
            ...
            rsyncCMD=" ".join(rsyncCMD.split())
            # self.logger.notify(rsyncCMD.replace(folder, yellowH+folder+colorReset).replace(dry_run, redH+dry_run+colorReset))
            self.logger.notify(rsyncCMD.replace(folder, yellowH+folder+colorReset))
            # self.logger.notify(rsyncCMD_color)
